...
int minimaxFree(board *b, bool isMaximising, int alpha, int beta, int depth)
{
	...
	if (isMaximising)
	{
		...
		for (int i = 0; i < halfOfPaths; i++)
		{
			...
			if (!cellWon(b, position)
				&& !isCellFull(*b->cells[returnRow(position)][returnColumn(position)]))
			{
...
int minimaxRestricted(board *b, bool isMaximising, int cell, int alpha, int beta, int depth)
{
	...
	for (int j = 0; j < 9; j++)
	{
		if (b->cells[returnRow(cell)][returnColumn(cell)]->boxes[returnRow(j)][returnColumn(j)]
			== 0)
		{
			potentialPaths++;
		}
	...
	for (int x = 0; x < 9; x++)
	{
		if (b->cells[returnRow(cell)][returnColumn(cell)]->boxes[returnRow(x)][returnColumn(x)]
			== 0)
		{
			if (isMaximising)
			{
...
void bestMoveRestricted(board *b, int *positionResult, int cell)
{
	...
	for (int i = 0; i < 9; i++)
	{
		if (b->cells[returnRow(cell)][returnColumn(cell)]->boxes[returnRow(i)][returnColumn(i)]
			== 0)
		{
