...
int almostFinishedRows(cell c, bool isPlayer)
{
	...
	{
		bool pair1 =
			c.boxes[i][0] == c.boxes[i][1] && c.boxes[i][0] == targetValue && c.boxes[i][2] == 0;
		bool pair2 =
			c.boxes[i][0] == c.boxes[i][2] && c.boxes[i][0] == targetValue && c.boxes[i][1] == 0;
		bool pair3 =
			c.boxes[i][1] == c.boxes[i][2] && c.boxes[i][1] == targetValue && c.boxes[i][0] == 0;
...
int almostFinishedColumns(cell c, bool isPlayer)
{
	...
	{
		bool pair1 =
			c.boxes[0][i] == c.boxes[1][i] && c.boxes[1][i] == targetValue && c.boxes[2][i] == 0;
		bool pair2 =
			c.boxes[0][i] == c.boxes[2][i] && c.boxes[0][i] == targetValue && c.boxes[1][i] == 0;
		bool pair3 =
			c.boxes[1][i] == c.boxes[2][i] && c.boxes[1][i] == targetValue && c.boxes[0][i] == 0;
		if (pair1 || pair2 || pair3)
		{
			result++;
		}
...
int almostFinishedDiagonals(cell c, bool isPlayer)
{
	...
	int result = 0;
	bool diag1 =
		c.boxes[0][0] == c.boxes[1][1] && c.boxes[0][0] == targetValue && c.boxes[2][2] == 0;
	bool diag2 =
		c.boxes[0][0] == c.boxes[2][2] && c.boxes[0][0] == targetValue && c.boxes[1][1] == 0;
	bool diag3 =
		c.boxes[1][1] == c.boxes[2][2] && c.boxes[1][1] == targetValue && c.boxes[0][0] == 0;
...
int evaluateCell(board *b, int i)
{
	...
	if (isCellFull(*b->cells[returnRow(i)][returnColumn(i)]))
		return 0;
	int AIPairs = 2 * (almostFinishedRows(c, false) + almostFinishedColumns(c, false)
		+ almostFinishedDiagonals(c, false));
	int playerPairs = 2 * (almostFinishedRows(c, true) + almostFinishedColumns(c, true)
		+ almostFinishedDiagonals(c, true));
